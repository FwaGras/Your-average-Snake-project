...
def computeNextFrame(numFrame,coordonnee, pomme_verte, pomme_rouge, pomme_marron):
    global direction
    ...
    if len(coordonnee) > 0:    
        ...
        for p in range(len(pomme_verte)):
            if coordonnee[0][0] == pomme_verte[p][0] and coordonnee[0][1] == pomme_verte[p][1]:
                ...
                if pomme_verte[p][0] == pomme_rouge[p][0] and pomme_verte[p][1] == pomme_rouge[p][1]:
                    pomme_verte[p][0] = randint(1,24)* 20
                    pomme_verte[p][1] = randint(1,24)* 20
        ...
        for p in range(len(pomme_marron)):
            if coordonnee[0][0] == pomme_marron[p][0] and coordonnee[0][1] == pomme_marron[0][1]:
                coordonnee.clear()
    ...
    for p in range(len(pomme_marron)):
        can.create_oval(pomme_marron[p][0], pomme_marron[p][1], pomme_marron[p][0] + 20, 
                         pomme_marron[p][1] + 20, outline= 'red', fill= 'brown') # Pomme marron
    ...
    if game_over :
        ...
        tk.after(100, lambda:computeNextFrame(numFrame,coordonnee, pomme_verte, pomme_rouge, pomme_marron))
...
if __name__ == "__main__":
    ...
    pomme_rouge = []
    # Troisième objet (la pomme létale)
    pomme_marron = []
    
    
    
    # Construction de la première étape de simulation
    computeNextFrame(0,coordonnee, pomme_verte, pomme_rouge, pomme_marron)
